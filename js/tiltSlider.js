...
;( function( window ) {
	...
	TiltSlider.prototype._addNavigation = function() {
		// add nav "dots"
		this.navy = document.createElement( 'navy' )
		var inner = '';
		for( var i = 0; i < this.itemsCount; ++i ) {
			inner += i === 0 ? '<span class="current"></span>' : '<span></span>';
		}
		this.navy.innerHTML = inner;
		this.el.appendChild( this.navy );
		this.navDots = [].slice.call( this.navy.children );
	}
	...
	TiltSlider.prototype._showItem = function( pos ) {
		...
		var cntAnims = 0,
			...
			onEndAnimationCurrentItem = function() {
				++animEndCurrentCnt;
				var endFn = function() {
					classie.removeClass( currentItem, 'hidee' );
		...
		classie.addClass( currentItem, 'hidee' );
